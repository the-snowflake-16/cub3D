#include "../include/cub3d.h"

int	ft_matrixlen(char **m)
{
	int	i;

	i = 0;
	while (m && m[i])
		i++;
	return (i);
}

t_list	*get_anim(t_img *img, t_list **anim, int n[0][2])
{
	if (n && anim && *anim && img)
		(*n)[0]--;
	if (!img)
		return (*anim);
	if (!img->i)
	{
		free(img);
		(*n)[1] = 0;
		return (*anim);
	}
	ft_lstadd_back(anim, ft_lstnew(img));
	return (*anim);
}

void	check_textures(char *trim, t_game *g, int n[0][2])
{
	char	**dir;

	if (!trim)
		cub_perror(inv_map, g, NULL, 1);

	dir = ft_split(trim, ' ');
	if (!dir || !dir[0] || !dir[1])
	{
		free(trim);
		ft_free_matrix(&dir);
		cub_perror(inv_map, g, NULL, 1);
	}
	if (!ft_strncmp(dir[0], "NO", 3))
		g->tex.n_bak = get_anim(mlx_load_img(g->mlx, dir[1]), &g->tex.n, n);
	else if (!ft_strncmp(dir[0], "SO", 3))
		g->tex.s_bak = get_anim(mlx_load_img(g->mlx, dir[1]), &g->tex.s, n);
	else if (!ft_strncmp(dir[0], "EA", 3))
		g->tex.e_bak = get_anim(mlx_load_img(g->mlx, dir[1]), &g->tex.e, n);
	else if (!ft_strncmp(dir[0], "WE", 3))
		g->tex.w_bak = get_anim(mlx_load_img(g->mlx, dir[1]), &g->tex.w, n);
	else if ((!ft_strncmp(dir[0], "F", 2) && g->tex.floor == -1) || \
		(!ft_strncmp(dir[0], "C", 2) && g->tex.ceiling == -1))
		get_cf_color(dir, g);
	else
	{
		ft_free_matrix(&dir);
		cub_perror(inv_map, g, NULL, 1);
	}
	ft_free_matrix(&dir);
}

void	read_map(char *file, t_game *g)
{
	char	*line[2];
	int		n[2];

	n[0] = -1;
	n[1] = -1;
	g->fd = open(file, O_RDONLY);
	cub_perror(inv_file, g, file, g->fd < 0);
	while (1)
	{
		line[0] = get_next_line(g->fd);
		if (!line[0])
			break ;
		line[1] = ft_strtrim(line[0], "\n");
		free(line[0]);
		if (line[1] && line[1][0] && ++n[0] < 6)
			check_textures(line[1], g, &n);
		else if ((line[1] && line[1][0]) || n[0] >= 6)
			if (!g->map)
    			cub_perror(inv_map, g, "g->map is NULL", 1);
			g->map->map = ft_extend_matrix(g->map->map, line[1]);
		if ((int)ft_strlen(line[1]) > g->width)
			g->width = ft_strlen(line[1]);
		free(line[1]);
	}
	cub_perror(empty_file, g, NULL, !n[0]);
	cub_perror(inv_tex, g, NULL, !n[1]);
	g->height = ft_matrixlen(g->map->map);
}

void	check_map(t_game *g)
{
	int	j;
	int	i;
	int	w;

	j = -1;
	while (++j < g->height)
	{
		w = ft_strlen(g->map->map[j]) - 1;
		i = 0;
		while (g->map->map[j][i] == ' ' && i <= w)
			i++;
		while (g->map->map[j][w] == ' ' && w > 0)
			w--;
		if (ft_strncmp(g->map->map[j], "", 1) == 0)
			cub_perror(inv_map, g, NULL, 1);
		if ((j == 0 || j == g->height - 1) && ft_strlen(g->map->map[j]) - \
			ft_countchar(g->map->map[j], ' ') - ft_countchar(g->map->map[j], '1'))
			cub_perror(inv_wall, g, NULL, 1);
		else if (w > i && (g->map->map[j][i] != '1' || g->map->map[j][w] != '1'))
			cub_perror(inv_wall, g, NULL, 1);
	}
	check_elements(g);
	cub_perror(inv_map, g, NULL, !j);
}
