...
int	get_mini_color(t_game *g, int len, int xy[2])
{
	...
	color = 0xFF000000;
	if (xy[0] < len && g->map->map[xy[1]][xy[0]] == ' ')
		color = 0xFF000000;
	else if (xy[0] < len && g->map->map[xy[1]][xy[0]] == 'c')
		color = 0x00FF0000;
	else if (xy[0] < len && g->map->map[xy[1]][xy[0]] == 'o')
		color = 0x0000FF00;
	else if (xy[0] < len && g->map->map[xy[1]][xy[0]] != '1')
		color = 0x005E5C64;
	else if (xy[0] < len)
		color = 0x008BB5F8;
	return (color);
...
void	cub_minimap(t_game *g)
{
	...
	xy[1] = -1;
	while (++xy[1] < g->map->row)
	{
		xy[0] = -1;
		len = ft_strlen(g->map->map[xy[1]]);
		while (++xy[0] < g->map->col)
